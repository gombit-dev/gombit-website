...
Every framework within a table was measured on one host, closed-loop, under fixed resource limits. The tables come from three separate runs of very different cost, so each states the commit, host and toolchain **it** was measured at and figures are not comparable across tables. Read [benchmarks/docs/methodology.md](https://github.com/gombit-dev/gombit/blob/main/benchmarks/docs/methodology.md) — especially its "How not to interpret these results" section — before citing any figure.
